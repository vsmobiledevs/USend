...
import com.base.network.model.CreateSubscription
import com.usend.R
...
class SavedCardsViewModel(
    ...
    fun subscriptionSquareUp(createSubscription: CreateSubscription,cardID:String,plant_type:String,amount:Float) {
        when {
            !mContext.checkInternetConnected() -> status.value =
                Resource.NoInternetError<Int>(R.string.default_internet_message)
            else -> {
                status.addSource(
                    repository.createSubscriptionSquareUp(SQUARE_UP_TOKEN, createSubscription,cardID,plant_type,amount)
                )
                {
    ...
    fun delete(card_id: String) {
        ...
        when {
            !mContext.checkInternetConnected() -> status.value =
                ...
                status.addSource(
                    repository.deleteSquareCard(SQUARE_UP_TOKEN, card_id)
                ) {
